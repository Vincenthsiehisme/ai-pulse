---
id: "src-kol-simonwillison"
type: "source"
owner: "Simon Willison"
media_group: "Simon Willison's Weblog"
person_id: "person-simon-willison"
track: "kol"
tier: 2
role: "expert"
...
# Simon Willison（src-kol-simonwillison）
...
|---|---|---|
| 收錄 | `probing` | 會被抓 |
| 已觀測 | 151 筆 | 抓到了，但站方那陣子沒發東西 |
| 有效產出 | 7 則事件 | 抓到了但聚類沒把它綁成證據 |
| 已發布 | 5 則 | 綁上了但門禁擋著——那是設計，不是故障 |

> 這條來源不能單獨作為一手證據（`can_satisfy_primary: false`）。它的角色是佐證與獨立性，不是「事情發生了」的來源。

> 媒體集團：**Simon Willison's Weblog**。獨立性是按 source + author + media group 判的，所以同一個 media_group 的兩條來源**加起來只算一個獨立聲音**。
