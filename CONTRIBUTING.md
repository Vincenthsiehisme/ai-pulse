...
`.github/workflows/data-refresh.yml` **一天跑一班**（cron `0 16 * * *` ＝ 台北隔日
00:00），抓取→評分→聚類→門禁→渲染，然後 `chore: nightly refresh` 直接推 `main`。
它動到的是：
...
這些是鏈自己的產物。把它們改成開 PR 會有兩個後果：每天多一個沒人會看的 PR 持續
累積；而且下一班的 rebase 基準永遠停在沒合併的狀態，鏈直接卡死。**後面那個才是致
命的**——就算一天只有一班，只要有一班沒被合併，下一班就接不上去。所以這條路刻意
留著直推，**不是漏掉，是設計**。

> **班次上限是每週自癒的，不要手動調高。** `data-refresh.yml` 另排了一個每週禮貌
> 檢查（`0 16 * * 0` UTC ＝台北每週一 00:00），它不看收班任務跑了沒，只讀那個檔案
> 的事實：**cron 一天超過一班、或任一處 `--stale-days` 小於 7，它就當場改回來並推
> `main`**。所以把 cron 改密不會報錯，只會在下週一被安靜地改回去——改之前先改那個
> 守衛，不然你會以為自己改成功了。
