...
標題與日期是 `pulse-render.load_corpus_index()` 從 `_corpus/` 現查的。**改成滾動
視窗會靜默打壞所有舊事件的那一區**——頁面照印，只是標題變成來源名、日期全部退成
事件日。解法不在這條：`references/event-timestamps.md`〈第三個現場〉要求那兩個欄位
優先讀 Event 自己的 `evidence[]`，做完之後這條才是一個可以自由決定的問題。
