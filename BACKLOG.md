...
| [`mutate-跑的時候可以改檔案`](#mutate-跑的時候可以改檔案) | 變異測試會把檔案改掉再改回來，中途編輯會落在那個窗口裡；已經發生兩次，兩次都是 `git status` 抓到的 | 不會 | 否 |
...
**2026-07-28 與 07-29 各發生一次**，各留下兩個與一個檔案在變異狀態
（`pulse-narrative-apply.py`、`lib/gate_keys.py`）。兩次都不是想起來的，是
`git status` 印出來才看到。第一次之後我把「跑的時候不要改檔案」寫進
mutation-inventory 第二十八輪；**第二十九輪就再犯一次**。

所以修法不是再寫一次那句話——這份清單裡已經有一條前例在講「靠人記得的機制不算
機制」（〈現況〉那一節：把量測時間寫進標題、請下一個人複量，三小時就證明不夠）。
可行的做法有兩個，都還沒做：
