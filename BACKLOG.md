...
### 2026-09-08 複量：這條已經不是「逼近零」，是**已經翻過去至少兩次**

上面那段寫的時候，最差是 +102 分。逐 commit 重量（`ai-pulse-bot` 的
`chore: nightly refresh` 對 `ai-pulse-enrich` 的 `nightly: enrich …`，
2026-08-26 起連續 13 晚）：

```
中位數約 +20 分
+14 / +15 / +16 / +19 分  四晚（08-29、09-02、09-03、08-30）
-118 分                   08-31   ← 順序翻過去
-16  分                   09-07   ← 順序翻過去
08-27 那晚 Actions 根本沒收工（延到 08-28 00:39），潤稿端自己補跑 probe（10 筆）
```

重量指令（一次性，這份檔案不留會過期的數字）：

```bash
git log --format='%ad|%an|%s' --date=format:'%Y-%m-%d %H:%M' --since=<起日> origin/main
# 每天各取 bot 的 'chore: nightly refresh' 與 enrich 的 'nightly: enrich' 第一筆，相減
```

**順序翻過去的後果不只一個，而第二個是新的。** 已知的第一個是 runbook 步驟 0
在防的那個（潤稿端 clone 到還沒收工的 repo，於是自己補跑 probe——08-27、09-07
兩晚都真的觸發了）。第二個是 2026-09-07 那晚踩到的：`_probe/title-zh-worklist.json`
是 Actions 那班重生的，Actions 晚到，潤稿端就讀到**昨天**那份清單——而昨天那份
列的正好是昨晚剛翻完的那幾則，照 runbook 字面逐條翻就會全部重翻覆蓋。
`fix/title-apply-idempotence` 已經在寫入那一刻擋住傷害（`overwrite_verdict()`），
**但成因沒動**：步驟 0 的前置檢查只看 `_corpus/<今天>/` 在不在，待譯清單、
GitHub 榜單待譯清單這些同樣是「Actions 產、潤稿端讀」的快照都不在它的視野裡。

所以這一條要做的還是原本那句：**把餘裕變成一個被記錄的量**——它現在只有在
翻車的那一晚看得見，而過去 13 晚已經翻了兩次，沒有任何一次被記下來。
順帶一提，餘裕為負時「潤稿端讀到的清單是哪一班寫的」也該一起記，
那才是分得出「這一晚安全」與「這一晚剛好沒東西可覆蓋」的欄位。
...
`scripts/mutate.py` 的流程是「讀原檔 → 寫變異 → 跑 selftest → 寫回原檔」。這期間
工作區是**髒的**，而且是它自己弄髒的。人在這個時候編輯同一批檔案，改動會落在那個
窗口裡：mutate 寫回原檔時把人的編輯蓋掉，或者中斷時把檔案留在變異狀態。

**已經發生三次**，三次都不是想起來的，是 `git status` 印出來才看到：

| 時間 | 留在變異狀態的檔案 | 成因 |
|---|---|---|
| 2026-07-28 | `pulse-narrative-apply.py` | 人在跑的時候編輯 |
| 2026-07-29 | `lib/gate_keys.py` | 同上 |
| 2026-07-29 | `pulse-render.py` | **整份 mutate 被逾時砍掉，沒有人碰它** |

第一次之後我把「跑的時候不要改檔案」寫進 mutation-inventory 第二十八輪；
**第二十九輪就再犯一次**。所以修法不是再寫一次那句話——這份清單裡已經有一條前例
在講「靠人記得的機制不算機制」（〈現況〉那一節：把量測時間寫進標題、請下一個人
複量，三小時就證明不夠）。

第三次把可行的兩個做法分出了高下：

1. **mutate 開跑時鎖住**：寫一個 `.mutate.lock`，`mutate.py` 自己開跑前先檢查、
   結束時刪掉；有鎖還要跑就拒絕。擋得住「同時跑兩份」，擋不住人手動編輯，
   **而且對第三種完全無效**——被 SIGKILL 的行程連刪鎖的機會都沒有，只會多留一個
   孤兒鎖，把「上一次沒跑完」變成「這次跑不了」。
2. **跑在 worktree 副本上**：`git worktree add` 一份臨時樹，變異全部發生在那裡，
   主工作區完全不動。比較貴（每次多一次 checkout），但它是**結構上不可能**而不是
   「記得不要」，而且砍在任何一刻主工作區都一樣乾淨。**三種成因它都擋得住，所以
   要做就做這一個。**

## `版面沒有人量過真的幾何`

站台這一層現在只有**規則層**被測：CSS 字串裡有沒有那條規則、產出的 HTML 有沒有
掛那個 class。**沒有任何一條測試量得到「這個東西在畫面上的哪裡」。**

代價已經收過兩次，兩次都是人眼在截圖上看到的：

- 2026-07-27 時間軸的浮層蓋住日期欄頭。修法換了兩次才對——第一次修的是裁切，
  而截圖上的問題是遮蔽。
- 2026-07-28 首頁整個 hero 貼齊左邊界、h1 的 color 吃到 `--muted`。
  而 `selftest` 那條「首頁的大只由 `.hero.lead` 表達」**是綠的**——它在 CSS
  字串裡 grep `.hero.lead h1{font-size:var(--fs-display)`，那串字一直都在。

兩次的共同形狀是這個 repo 的老毛病換了一個位置：**用「規則寫對了」代理「畫面對了」**。
兩者在多數日子重合，正好在有人改版型的那天分岔。

查出第二次是靠 headless Chromium 量 `getBoundingClientRect()`：首頁 hero `x=0`，
其他四頁 `x=323`。**那是在本機手動跑的，CI 裡沒有這一步。** 這條 backlog 就是
...
長文版那一輪（PR2）又多兩個：頁首與版面鑑別線印同一句話（四頁各兩份）、
以及卡片改規則線之後有幾條 `background` 沒對到而留著填色。前者最後是變異抓到的，
後者是截圖抓到的。**到目前為止，站台的版面問題沒有一個是 selftest 先發現的。**
