...
| 名字 | 事 | 壞了會紅嗎 | 現在在騙人嗎 |
|---|---|---|---|
| [`判斷層自相矛盾`](#判斷層自相矛盾) | 一則已發布 Event 的內文說「單一獨立來源」，而它自己的 frontmatter 寫 `independent_sources: 2` | 不會 | **是，對外站上** |
| [`傳播因子印成 0`](#傳播因子印成-0) | 四個從未量測的因子在 **41 / 41** 則已發布頁上印成粗體 0 | 不會 | **是，對外站上** |
| [`榜單中文落地靠人工繞道`](#榜單中文落地靠人工繞道) | 譯文落地了一次，但機制還是壞的：`apply` 讀的榜單不在版控裡，fresh clone 一定退件 | 不會 | **是（我在上一版把它當成修好了）** |
...
**這一批裡唯一正在對讀者說假話的一條。** 排第一，因為這份清單自己的排序準則
第 2 條就是「它現在是不是正在輸出一個錯的數字」。

`evt-2026-07-21-1bdb1a`（`status: published`，對外站上看得到）：

```
frontmatter:  independent_sources: 2
判斷層內文：  「…（單一獨立來源，暫標待證實）」
```

**同一頁上兩個相反的宣稱。** 病灶在 `pulse-enrich-apply.py:77-80`：它依**當下**的
`independent_sources` 寫出那句話並烙進 prose，之後不再重算；而同一頁的警示框是
render 即時算的。第二個獨立來源進來之後，警示框消失了，那句話留著。

也就是**凍結的快照被當成現況**——跟 `_slug_to_title`、`lastmod` 當 `published`
是同一隻病，只是這次的代理是「寫下去那一刻的值」代表「現在的值」。

修法有兩個方向，要拍板：把那句話從 prose 拿掉、改由 render 即時產生（跟警示框
同一個真相源）；或保留 prose 但存一個 `judged_at` 與當時的值，讓它自己說得出
「這句話是那時候的」。**前者比較對**——判斷層本來就不該有第二個時鐘。

---

## `傳播因子印成 0`

`uniqueAuthors` / `platformBreadth` / `regionBreadth` / `velocity` 四個因子
**結構上永遠是 0**：`pulse-cluster.rescore()` 寫死 `metrics=[]`
（`scripts/pulse-cluster.py:333`）。

2026-07-27 逐頁量：**41 / 41 則已發布 Event 全部把這四格印成粗體 0。**

而 `lib/scoring.py` 自己的註解寫著：

> 不印 0——0 看起來像「量過了，很冷」，那是更難察覺的一種謊。

修 `heat` 那次照這句話做了（改印「未量測」），**但只做到純量那一層，
沒往外一格的因子看**。所以現在的狀況是：同一張卡片上，`heat` 誠實地寫著
「未量測」，旁邊四個因子理直氣壯地印著 0。

修法沒有爭議（改印「未量測」，跟 `heat` 同一個做法），要決定的只有一件：
**四個因子要不要真的接上**。接上要先有 `metrics` 的來源；不接就永遠印
「未量測」，那也是一個誠實的終態。**不可以繼續印 0。**
...
修法已經有人寫好了，就在 `fix/c2-apply-reads-committed-worklist`
（潤稿端 2026-07-27 19:29Z 自己開的分支）：榜單不在時退讀同一份
`_probe/github-desc-worklist.json`，並且**刻意不去捏一個殘缺的 dist 榜單**
（那會誤導同 session 後面的 prep）。`validate()` 一個字沒動。

**它現在的狀態是「躺在分支上」**，而這份清單第一行就寫著
「修好了但還沒併進 `main` 的，算沒修」。合併前要做兩件事：

1. **rebase 到現在的 `main`**——它的基底是 `d39697a`，已落後 12 個 commit，
   而且它動的 `BACKLOG.md` 跟 PR #33 衝突（它往〈榜單描述沒有中文〉那一節
   加註記，而 PR #33 把那一節整段移走了）。
2. **重跑 selftest**：它在自己的基底上是 526 / 526，現在的 `main` 是 585。
