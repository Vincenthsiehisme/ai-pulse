...
status: "green"
last_success: "2026-07-27"
probe_lag_days: 0
last_run_day: "2026-07-27"
run_lag_days: 0
stale_after_days: 2
sources_runnable: 27
items_observed: 619
...
> 這頁自己就是死人開關：鏈沒跑就沒人重寫它，`generated_day` 會停在 2026-07-27 不動。所以先看那個日期是不是今天，再看下面的燈。
...
| 已觀測 | 累計 619 筆，來自 25 條來源 | 這裡是**歷來**累計，含現在已停用的來源，所以不能直接跟上一格相減。本窗口誰零產出看下面「覆蓋範圍」那一節 |
| 有效產出 | 56 則事件 | 抓到了但沒聚成事件＝聚類沒認出來 |
...
| | 最後一次 | 距今 | 這條停了代表什麼 |
|---|---|---|---|
| 有跑班（`_probe/`） | 2026-07-27 | 0 天 | 排程死了 |
| 有抓到東西（`_corpus/`） | 2026-07-27 | 0 天 | 鏈在跑但瞎了（來源全壞、或全站沒更新） |

「靜默死掉」與「靜默瞎掉」是兩種病，所以兩條軸分開印。2026-07-24 漏抓 Claude Opus 5 的那晚，上面那條是綠的。

## 佇列

- 已上線 **36**／review **19**（待處理 8、設計上擋著 11）／人工判定不追 **1**
...
近 30 天（實有語料 4 天）。「沉默過久」是拿**每條實體自己的觀察期**判的（底下來源最早開始被觀察那天起算），不是拿語料庫長度——見 `references/health-alarms.md`。

| 必盯實體 | 來源 | 看見 | 事件 | 上線 | 最後看見 |
|---|---|---|---|---|---|
| OpenAI | 1 | 130 | 11 | 10 | 0d 前 |
| Anthropic | 1 | 32 | 8 | 6 | 0d 前 |
| Google DeepMind | 1 | 24 | 3 | 2 | 0d 前 |
| Google | 1 | 59 | 5 | 3 | 0d 前 |
| Meta | 1 | 22 | 0 | 0 | 0d 前 |
| Microsoft | 1 | 20 | 0 | 0 | 0d 前 |
| NVIDIA | 1 | 61 | 17 | 12 | 0d 前 |
| Hugging Face | 1 | 47 | 1 | 1 | 0d 前 |
| xAI | 1 | 4 | 2 | 2 | 0d 前 |
| Mistral AI | 1 | 2 | 0 | 0 | 0d 前 |
| Alibaba | 1 | 0 | 0 | 0 | **從未**（觀察期 3/30d，還沒到門檻，所以不判沉默） |
| DeepSeek | 0 | 31 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| Safe Superintelligence | 0 | 1 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| Thinking Machines Lab | 0 | 0 | 0 | 0 | **從未** ○ 已知未覆蓋（不觸警） |
| Cohere | 0 | 3 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| Perplexity | 0 | 0 | 0 | 0 | **從未** ○ 已知未覆蓋（不觸警） |
| Anysphere (Cursor) | 0 | 0 | 0 | 0 | **從未** ○ 已知未覆蓋（不觸警） |
| Cognition | 0 | 2 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| Scale AI | 0 | 36 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| Z.ai | 0 | 0 | 0 | 0 | **從未** ○ 已知未覆蓋（不觸警） |
| Moonshot AI | 0 | 3 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| MiniMax | 0 | 8 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
...
- ⚠ 榜單中文描述：0/25 條，**從來沒有成功翻過一次**（潤稿端 C2 段；缺工，不是故障）
