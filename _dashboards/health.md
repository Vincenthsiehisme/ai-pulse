...
> 這頁自己就是死人開關：鏈沒跑就沒人重寫它，`generated_day` 會停在 2026-09-03 不動。所以先看那個日期是不是今天，再看下面的燈。
...
| 已觀測 | 累計 2639 筆，來自 25 條來源 | 這裡是**歷來**累計，含現在已停用的來源，所以不能直接跟上一格相減。本窗口誰零產出看下面「覆蓋範圍」那一節 |
| 有效產出 | 256 則事件 | 抓到了但沒聚成事件＝聚類沒認出來 |
| 已發布 | 215 則 | 卡在門禁是設計，不是故障 |

## 鏈的兩條時間軸

| | 最後一次 | 距今 | 這條停了代表什麼 |
|---|---|---|---|
| 有跑班（`_probe/`） | 2026-09-03 | 0 天 | 排程死了 |
| 有抓到東西（`_corpus/`） | 2026-09-03 | 0 天 | 鏈在跑但瞎了（來源全壞、或全站沒更新） |

「靜默死掉」與「靜默瞎掉」是兩種病，所以兩條軸分開印。2026-07-24 漏抓 Claude Opus 5 的那晚，上面那條是綠的。

## 佇列

- 已上線 **215**／review **39**（待處理 24、設計上擋著 15）／人工判定不追 **2**
- 未 enrich **10** 則，最久放了 **0** 天
- 待處理卡最久 **35** 天（天數＝**進庫**多久，不是新聞發布多久）

| blocker | 則數 |
|---|---|
| `thin_research_analysis` | 12 |
| `stale_backfill` | 11 |
| `placeholder_content` | 10 |
| `missing_category` | 10 |
| `missing_track` | 10 |
| `thin_fact` | 9 |
| `thin_by_policy` | 6 |
...
|---|---|---|---|---|---|
| OpenAI | 2 | 1116 | 52 | 51 | 0d 前 |
| Anthropic | 1 | 416 | 33 | 32 | 0d 前 |
| Google DeepMind | 1 | 172 | 9 | 8 | 0d 前 |
| Google | 1 | 461 | 9 | 8 | 0d 前 |
| Meta | 1 | 227 | 1 | 1 | 0d 前 |
| Microsoft | 1 | 126 | 2 | 1 | 0d 前 |
| NVIDIA | 1 | 432 | 26 | 24 | 0d 前 |
| Hugging Face | 1 | 160 | 12 | 6 | 0d 前 |
| xAI | 1 | 14 | 8 | 7 | 0d 前 |
| Mistral AI | 1 | 2 | 0 | 0 | 1d 前 |
| Alibaba | 1 | 9 | 0 | 0 | 10d 前 |
| DeepSeek | 0 | 238 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| Safe Superintelligence | 0 | 0 | 0 | 0 | **從未** ○ 已知未覆蓋（不觸警） |
| Thinking Machines Lab | 0 | 3 | 0 | 0 | 4d 前 ○ 已知未覆蓋（不觸警） |
| Cohere | 0 | 26 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| Perplexity | 0 | 7 | 0 | 0 | 1d 前 ○ 已知未覆蓋（不觸警） |
| Anysphere (Cursor) | 0 | 0 | 0 | 0 | **從未** ○ 已知未覆蓋（不觸警） |
| Cognition | 0 | 6 | 0 | 0 | 13d 前 ○ 已知未覆蓋（不觸警） |
| Scale AI | 0 | 233 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| Z.ai | 0 | 8 | 0 | 0 | 6d 前 ○ 已知未覆蓋（不觸警） |
| Moonshot AI | 0 | 0 | 0 | 0 | **從未** ○ 已知未覆蓋（不觸警） |
| MiniMax | 0 | 54 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| ByteDance | 0 | 8 | 0 | 0 | 17d 前 ○ 已知未覆蓋（不觸警） |
| Baidu | 0 | 0 | 0 | 0 | **從未** ○ 已知未覆蓋（不觸警） |
| Tencent | 0 | 5 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| AMD | 1 | 45 | 0 | 0 | 0d 前 |
| TSMC | 0 | 0 | 0 | 0 | **從未** ○ 已知未覆蓋（不觸警） |
| Broadcom | 0 | 0 | 0 | 0 | **從未** ○ 已知未覆蓋（不觸警） |
| Groq | 0 | 12 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| Cerebras | 0 | 22 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
| CoreWeave | 0 | 3 | 0 | 0 | 24d 前 ○ 已知未覆蓋（不觸警） |
| AWS | 0 | 77 | 0 | 0 | 0d 前 ○ 已知未覆蓋（不觸警） |
...
- 判斷層帳本：146 筆，最近一次 2026-09-02（1 天前）；最近 7 天有 6 條主線改過主張（agent-refactor、capital-evolution、global-map、infra-cost、model-research、product-market）
