...
- 按掉的人／機制：human　時間：2026-07-25T00:20:00+00:00
- 當時的 blockers：thin_research_analysis, generic_entity, stale_backfill
- 理由：三個擋它的理由沒有一個會隨時間消失：(1) stale_backfill——這是 src-hf-blog 第一次抓取整批帶進來的存量，不是當期訊號；(2) generic_entity——這則的提出方是誰，證據本身沒寫，不像 Nunchaku 那則是字典有洞，補字典也救不回來；(3) thin_research_analysis——單一來源、方法細節未附， 影響與脈絡撐不出實質分析。硬要發它，等於用一則「有人做了一套語音評測、但不知道是誰做的」佔掉版面。 這不是刪除：檔案留著、證據留著，只是不進 published 也不再排隊等潤稿。日後若出現第二個獨立來源 講同一件事，把 status 改回 review 即可重新走門禁。
