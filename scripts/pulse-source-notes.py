...
def render(src, obs, obs_day, ev_n, pub_n, state, health):
    ...
    fm["first_fetch_at"] = _day((state or {}).get("first_fetch_at"))
    fm["last_observed_day"] = obs_day
    fm["items_observed"] = obs
    ...
    out += ["---", "",
            ...
            "| 層 | 數字 | 這一格是 0 代表什麼 |",
            "|---|---|---|",
            f"| 收錄 | `{src.get('lifecycle')}` |"
            f" {'會被抓' if src.get('lifecycle') in RUN_LIFECYCLES else '**不會被抓**'} |",
            f"| 已觀測 | {obs} 筆 | 抓不到，或站方那陣子沒發東西 |",
...
def main():
    ...
    if not args.quiet:
        ...
        blind = [s["id"] for s in srcs
                 if s.get("lifecycle") in RUN_LIFECYCLES and not obs.get(s["id"])]
        if blind:
            print(f"  ⚠ 會被抓但零產出 {len(blind)} 條：{', '.join(blind)}")
    return 0
