...
import pathlib
TARGET = pathlib.Path(__file__).resolve().parent / "verify-policy-sources.py"
if not TARGET.exists():
    raise SystemExit(f"cannot find {TARGET} -- keep this test next to the script it tests")
spec = importlib.util.spec_from_file_location("v", str(TARGET))
