...
2. **只根據提供的證據寫，不編造。** worklist 每個 Event 附了綁定證據的標題 + 摘要。prose 只能講
   證據支持的內容。**不要瞎掰**——假故事比空話更糟，空話只是無聊，編造是說謊。

   **但「沒有內容」有三種，不能寫成同一句話。** worklist 每一筆證據都帶 `availability`
   與 `writing_hint`，照它寫。規格見 `references/evidence-availability.md`。

   ```
   has_text    內容在手上 → 照證據寫
   unfetched   我們沒抓到 → **不要寫進文章**。這是故障不是證據狀態，
               文章裡就當這一則只有標題；把它寫進收尾摘要讓人去修抓取端
   withheld    我們不取（站方宣告 ai-input=no、或 license 只允許標題連結）
               → 寫出來並且給連結：說明為什麼不轉述、原文在哪裡
   ```

   「（證據不足，待補）」只留給 `has_text` 而內容真的很薄的那一種——來源給了，
   而它給的就是這麼多。

   2026-08-13 踩到的：Grok Bot 那則三層寫「（證據不足，待補）」，下一個訊號寫
   「xAI 是否釋出功能說明」，而那份說明就在 `https://x.ai/bot`——連結躺在它自己的
   證據列裡。**系統在等一個它手上已經有的東西。** 空摘要的理由不是頁面沒內容，
   是我們照 `ai-input=no` 選擇不取。政策決定被寫成了證據狀態。
