...
```
rows                     250        （2024-05-10 → 2026-07-24，26 個月）
...
把錨點規則改壞之後（M112 原樣套用）37 / 128。** 九倍的分離度，這個守衛才有用。
...
## 5‴. 第二輪審查：三個洞**全部長在第一輪的修正裡**

同一位審查者對修正本身再看一次。三個發現，形狀高度一致：
**兩個是「拿一個關於世界的假前提去修」，一個是過度修正。**

| # | 洞 | 假前提 / 過度 | 實測後果 |
|---|---|---|---|
| 1 | `derive_versions` 的 `bare` 過濾器 | 註解寫「被別的衍生實體**字首包含**」，碼裡**沒有任何包含判斷** | 「retired the Claude 2.0, Claude 2.1, and Claude Sonnet 3」的前兩個被刪——**兩則真的下線宣告**；`GPT-4 and Claude Opus 5` 只剩後者 |
| 2 | 詞首加 `\b` 擋 model id 碎片 | 「`\b` 擋得住 `claude-3-7-sonnet-…`」——**加不加結果一模一樣**，那裡本來就有詞界 | 沒修到任何東西，卻讓字典裡 **8 個中文 alias** 在中文散文裡全部失效（「發布通義千問 3」的「布」把詞界吃掉） |
| 3 | `og:title` 改用 `[^>]*?` | 註解寫「屬性值裡的 `>` 在合法 HTML 裡是 `&gt;`」——**HTML5 只禁止同款引號與有歧義的 `&`** | `content="Claude 3 -> Claude 5"` 抽不到 → `no_og_title` → exit 3 →「這一站沒有 og:title」。**又一次把「我們讀不到」寫成「站方沒有」** |

第 2 項最值得記：一個為了修 A 而加的東西，**A 沒修到，B 全死**，
而兩件事都沒有任何測試會紅——那條測試（「不得切出 `claude@3`」）之所以綠，
是因為第 1 項的過度修正剛好把它清掉了。**兩個 bug 互相掩護，測試在上面睡著。**

真正擋得住碎片的是**尾端邊界**：比對停在 `claude-3`、後面緊接 `-7`。
散文裡的「Claude 3.5」會把版本吃完，後面是空白或標點。

順帶修掉的四個小的：月份 regex 的 `[a-z]*` 讓 `Marketing 2026` = mar + keting
（在 `is_date_only` 上代表真條目被當目錄項丟掉）；icon font 的私用區字元讓
`\ue09a July 24, 2026` 判不出是純日期（實測 124 條標題全部變成條目）；
區間尾端沒收尾的 `<div class="…"` 碎片變成條目文字；未登記的 verdict
預設成 2（站方的答案）而不是 4（沒有判決）。

條目切分也不再押「條目一定住在 `<li>` 裡」——那個前提只在**已經看過的那一頁**
成立，而另外三家的標記形狀還沒有人看過。現在兩邊都收、`<nav>/<footer>` 先剝掉。
真頁面 248 → 250 列（多的兩列是真的非 `<li>` 前導句），五個比率一格未動。

**這一輪也出現這條分支上第四次「以為守著、其實沒有」**：補的那條測試寫成
`VERDICT_EXIT.get("some_future_verdict", 4) == 4`——那是在測 `dict.get`
的預設值行為，不管碼怎麼寫都會過。真正的判斷躲在 `main()` 裡沒人叫得到，
所以拉成 `exit_code()` 才測得到。
