...
**只寫到「日」。** 帶時分秒的欄位會讓每一班都產生一次沒有資訊量的 diff，真正的
變化被埋掉。加密期實測過：33 個 Sources 頁 × 一天 12 班 ＝ 396 次無意義的檔案異動。
所以 `robots_checked_at` 只取前 10 個字元存成 `robots_checked_day`，健康頁只有
`generated_day`。

**這條不綁班次。** 現在一天一班（`0 16 * * *`），但班次是會變的——它加密過一次，
還會再加密。**規則寫成「輸出不要跟班次綁在一起」，就不會隨著班次改動而過期**；
寫成「因為一天 12 班所以要怎樣」的版本，收班那天就變成假的了。

**內容沒變就不重寫檔案。** 同一天內的後續班次通常什麼都不寫。
...
## `_dashboards/signal-review.md`

`pulse-signal-review.py --write` 產生。**待回答清單，不是歷史。**

這一頁列的是「我上次說要看什麼」——主線層的 `next`（`_config/narratives.yaml`）
與事件層的「## 下一個訊號」，兩層都收，因為它們回答的是不同尺度的問題：
主線那句問「這條線接下來看什麼」，事件那句問「這一則後續如何」。只收一層的話，
報告寫得出趨勢卻對不回具體事件，或者反過來。

### 它刻意不判定任何事

2026-08-11 實測：86 則的「下一個訊號」約六成以「看…」開頭——是寫給人的觀察
指示，不是機器可判定的命題；而且指向 benchmark 數字、出貨時程、第三方採購行為
這類**這條抓取鏈看不到的世界**（32 條來源裡 16 條是官方部落格）。同期
81/86 的事件仍然只有一個獨立來源，連最機械的那種後續都只發生在 6% 上。

所以這一頁只整理問題、由人回答。理由全文見
`docs/design/2026-08-11-theme-tracking-revisited.md`。

### 裁決帳本

答案 append 進 `_probe/signal-verdicts.jsonl`，走 `lib/history.py`（與來源層、
判斷層同一個模組、同一個形狀）。三個值：
...
| `happened` | 那個訊號出現了 |
| `not-yet` | 還沒，繼續等 |
| `unanswerable` | **這個系統看不到它** |

`unanswerable` 不是失敗，是這一頁最有價值的產出：累積起來就是「觀測範圍的
缺口清單」，而那份清單才是決定「要不要做自動核對、該先接哪一類來源」的依據。

### sid 綁內容不綁流水號

`<來源鍵>#<內容雜湊前 8 碼>`。那段話被改寫過就是**另一個問題**，舊答案不該
繼續掛在上面——改寫後它會以新的 sid 重新排進待答清單。同 `lib/zhtext.py`
的 `src_hash()`。

已回答的不再列進頁面：**這一頁是待辦，歷史在帳本裡。**
...
2026-08-11 P0-a 第一批 26 則裁決之後，立刻看到一個問題：`benchmark` 有 4 條供給，
判成 🟡。但那 4 條裡 **3 條是官方線**——OpenAI、Anthropic、Hugging Face 自己評自己。
真正獨立的只有 Interconnects 一條。7 個需求對 1 條獨立供給，那不是 🟡。

所以供給拆兩欄：

```
官方   track: official           當事人自己說
獨立   track: media / kol        不是發布方的人說
（track: aggregator 不進任一欄——規格寫明它「只當候選來源，不作任何事實依據」）
```

**燈號改由「獨立」那一欄算。** 理由不是理論，是這 26 則量出來的：
22 則判 `unanswerable` 的，**每一則都是「只有官方說了，我們判定看不到」**。
官方供給在這批需求上的實測命中率是 0——再多官方來源也補不上這些格子。

`獨立 = 0` 而 `官方 > 0` 的那幾列會另外標記「只有當事人自己在說」，
因為那是最容易被誤讀成「有覆蓋」的形狀。

**這一拆改不了的事**：`enterprise_adoption` 拆完是官方 2／獨立 5，
獨立那邊其實不缺。它的 4 個需求答不出來，不是因為沒有獨立來源，
是因為那 5 家報的是新聞不是採用數字——**題材對了、粒度不對**。
這張表量不到粒度，下一輪要不要量、怎麼量，等更多裁決再說（紅線 8）。

### 分母與需求都取「現值」，不是每一筆

帳本是 append-only，所以**同一則的現值是最後一筆**（`lib/history.latest`）。

2026-08-11 之前這一頁是把每一筆 `unanswerable_reason` 都數進去的。26 則一次答完
的時候，兩種算法給一樣的答案——所以它看起來是對的。它會在**第一次有人改答案
的那天**開始靜靜多算，而且不會有任何東西變紅。那是「錯得慢」的形狀：
一直是個合理的數字，只是慢慢地偏。

而且只數「現在還是 `unanswerable`」那幾則的原因碼：一則從 `unanswerable` 改判成
`happened` 之後，它的舊原因碼不該繼續佔著矩陣上的一格需求。

`from` / `to` 這兩個欄位存在的理由就是讓「改過」看得見；
下游不分現值與歷史的話，那個設計就白做了。
