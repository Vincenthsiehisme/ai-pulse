...
**這條自保自己也要有樣本。** 「超過一半」在一兩支分支上沒有分辨力：2026-09-15
把四支夜班分支刪掉之後，origin 上只剩一支進行中的 PR 分支，那一支還沒合併是
**正常狀態**，而 1 支沒收就是 100%，判準當場宣告自己失效並觸警。squash 的症狀是
每一支歷史分支同時看起來沒收，樣本太小根本看不出那個形狀。所以分支總數少於
`SUSPECT_MIN_BRANCHES`（起手 4）時不做這個判斷。這個數字是照分辨力訂的，
不是量出來的分布，要調走 PR。
...
ok           量到了
no-git       這裡不是 git 工作區
no-remotes   單分支 clone（refspec 只抓一條）→ **量不到**，不是 0
suspect      不在 main 的超過一半 → 判準可能失效（見上）
```

**一支都看不到，還要再分一次。** 「真的都收乾淨了」跟「這個工作區看不到別的
分支」兩種都會算出空清單，而要做的事相反：前者不用做任何事，後者要去補
`fetch-depth: 0`。分辨的依據是 refspec：`+refs/heads/*:refs/remotes/origin/*`
代表這個工作區看得到全部分支，那就是真的 0 支；`+refs/heads/main:...`
只抓一條，那才是量不到。

這個歧義在 2026-09-15 之前是看不見的，因為下一段那個 `origin/HEAD` 的濾漏
讓 `no-remotes` 那條路從來沒走到過。修好濾漏之後它立刻就會踩到：**2026-09-15
把四支夜班分支全刪掉，origin 上只剩 main**，不分這一層的話，Actions 那一邊
（`fetch-depth: 0`，看得到全部）會印出「量不到，這是單分支 clone」——
一句在那個位置上剛好是錯的話。
