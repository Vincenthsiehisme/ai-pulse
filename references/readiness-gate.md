...
heat / value 下架之後，把剩下的欄位對 86 則已發布事件量了一次分佈：

| 欄位 | 相異值 | 實況 |
|---|---|---|
| `impact` | **1 種** | 86/86 都是 55 |
| `authority`（權威） | **1 種** | 86/86 都是 90 |
| `uniqueAuthors`（獨立作者） | **1 種** | 86/86 都是 None |
| `confidence` | 6 種 | 79/86 是 73（92%） |
| `corroboration`（佐證） | 4 種 | 81/86 是 20 |
| `primaryEvidence`（一手證據） | 2 種 | 81/86 是 50 |
| `independentSources`（獨立來源） | 4 種 | 81/86 是 1 |
| `freshness`（新鮮度） | 44 種 | 真的有分佈 |

**一個在每一則上都相同的數字，佔著版面卻不帶資訊。** 而且它比「未量測」更糟：
「未量測」至少誠實地說了自己沒有內容，一個常數看起來像量出來的。

### `impact` 從來沒有被計算過

`impact` 是 `scoring.score_event(..., impact_hint=55)` 的預設值，而**整條鏈沒有
任何呼叫端傳過那個參數**。所以 86/86 都是 55 不是「大家剛好一樣」，是它一次都
沒有被算過。它下架，而且不是因為它「不重要」——是因為它現在不是一個量測結果。

接上真的輸入是 `scoring.py` 的題目，不是畫面的題目，寫進 BACKLOG。接上之後
要回到這一節重新決定顯不顯示。

### `authority` 講的是語料庫，不是這一則

86/86 都是 90，因為這個系統的來源清一色是官方部落格。那是一個關於**整個語料庫**
的事實——「我們只收一手官方來源」——屬於方法說明，不屬於每一則事件的評分理由。
每一則都印一次，只是把一句話重複 86 遍。

`uniqueAuthors` 跟四項傳播輸入同一類（永遠 None）。上一版留著它，理由是
「它有量測管道、只是這則沒量到」——實測 86/86 全是 None，那個理由不成立。

### 留下來的四項，就是「為什麼這則是 83 不是 73」

佐證 / 一手證據 / 獨立來源 / 新鮮度。這四項會隨事件不同，而且它們合起來正好
就是 `confidence` 的組成。「評分理由」這個標題到這一版才名副其實。

**判準要守兩個方向**：常數欄位不准回到版面（`authority` / `uniqueAuthors`），
以及這四項必須還在——只守前者的話，「只顯示會變的」會退化成「什麼都不顯示」。
