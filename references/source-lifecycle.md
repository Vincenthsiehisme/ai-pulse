...
|---|---|---|
| `200` | 成功 `+8` | — |
| `200` 但 0 筆 | **成功** `+8` | 安靜的 feed 是健康的 feed。把「沒新聞」罰成失敗，等於逼系統偏好吵的來源。**但這一列只看單班**——累計那一面見下方補註。 |
...
### 這張表只看單班，累計那一面 2026-08-11 才有人量

上面那一列原本舉的例子是「`src-mistral-news` 連兩天 200／0 筆」。
2026-08-11 回頭量，它不是連兩天：

```
src-mistral-news   runs=25  consecutive_successes=24  last_status=200  score=100
                   _corpus/ 累計相異項目數：0        ← 一次都沒有過
```

**判斷仍然是對的**——不該因為安靜就扣分。錯的是沒有人在看第二個維度：
一班 0 筆是安靜，25 班全部 0 筆是「這條線從來沒有到過貨」，而這兩件事
在健康分上長得一模一樣（都是 100）。

同樣的形狀有三條：`src-mistral-news`（200／0 筆）、`src-media-theregister`
（`robots_disallow`）、`src-kol-thezvi`（`robots_unknown`）。後兩條是合規原因，
中性不記分也是對的，代價一樣是永遠 100 分、永遠不進隔離候選。

補的是消費者不是判準：`pulse-monitor` 的〈來源能力〉那一行每班印出
「幾條宣稱了但語料裡從來沒有過」。它**不觸警**，理由見
`references/source-capabilities.md`〈這一格今天不觸警〉——摘要是三條裡有兩條
的成因是合規、會永遠紅，而一個永遠紅的警報兩週內就會被關掉，
連真的該追的那一條也一起靜音。

**下一輪的題目**：把「合規上抓不到」與「抓得到但一直空手」分成兩種狀態。
分得開才有辦法只對後者叫。
