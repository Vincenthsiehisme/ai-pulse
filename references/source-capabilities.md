...
沒有人看得到跟進  → 後續存在，但這 33 條來源裡沒有一條的職責是報那種東西
```
...
今天 33 條的標註是照 `_corpus/` 裡實際抓到的標題標的（25 條有語料），
...
16 個值，定義在 `scripts/lib/sources.py` 的 `CAPABILITIES`。
...
### `oss_release` 為什麼不併進 `product_release`

2026-08-22 補（`references/incidents/2026-08-22-the-surface-nobody-watched.md`）。

兩者分開，因為它們**被驗證的方式不同**：

```
product_release  有東西可以用了 → 你能用，但你不能檢查
oss_release      原始碼／權重公開了 → 第三方可以自己跑、自己驗、自己改
```

這個軸這張表本來就在用了，只是缺了供給端那一半：`research_replication`
（有人試著重現）已經是一個能力，而**開源釋出正是重現的前提**。
沒有 `oss_release`，`research_replication` 這一列的需求永遠對不到它真正缺的那個東西。

還有一個現成的證據說這個字本來就該在詞彙表裡：`oss_release` 早就是
`corpus_type` 的合法值（`sources.yaml` 檔頭那張表）。同一個概念已經在 repo 裡了，
只是掛在描述欄位上——而描述欄位不進覆蓋率矩陣。

### 標註規則：職責，不是曾經報到過

標 `oss_release` 的判準是**「這條來源的既有職責裡本來就包含開源釋出」**，
不是「它曾經報導過一次」。

這條規則是被逼出來的：08-22 掃全語料，15 條來源的標題裡出現過開源相關字眼，
其中一半是偶然命中（`src-ep-itre` 那則是歐盟國防新聞稿裡剛好有 "press release"）。
照「報到過就標」去標，獨立供給那一欄會虛胖，而燈號只看那一欄。

### 一次刻意不標：`src-openai-blog`

`src-openai-blog` **沒有**標 `oss_release`，而 OpenAI 確實會開源。

理由是實測而不是判斷：08-19 那則開源公告發在 `developers.openai.com/blog`，
而這條來源訂的是 `openai.com/news/rss.xml`——把整份 200 則 feed 拉下來對過，
**那則不在裡面**。標上去等於把一次量到的失敗寫成一個宣稱，而這一頁開頭
就在講「capability 是宣稱，三個月後會被當成測量來引用」。

`src-anthropic-news` 同樣沒標，理由不同：語料裡唯一的命中是
「Position Open Weights Models」，那是政策立場，不是釋出。
...
`pulse-monitor` 每班印一行對照（2026-08-22 實跑）：

```
來源能力：running 28 條，全數已標；16 種能力裡 1 種沒有任何來源宣稱（procurement）；
3 條宣稱了但語料裡從來沒有過（src-kol-thezvi、src-media-theregister、
src-mistral-news）；1 條還沒跑過第一班（src-gh-openai-codex），先不算它兌不了現
```

### 第四格是被 08-22 那次補來源逼出來的

加 `src-gh-openai-codex` 的那一刻，這一行從「3 條宣稱了但從來沒有過」
變成 **4** 條——而第 4 條的成因跟前三條完全不同：

```
前三條  跑了 25 班、健康分 100、一筆都沒有過   → 宣稱兌不了現
第四條  一班都還沒跑                            → 還沒輪到它
```

把「沒有」跟「還不知道」寫成同一個數字，正是這個 repo 一直在修的病
（見 `references/evidence-availability.md`），而它出現在一個**專門用來抓
「宣稱兌不了現」的指標**上。

代價很具體：這樣算的話，**每一次補來源都會先製造一筆假的壞消息**。
讀的人學會忽略它之後，前三條那種真的該追的也一起沒人看——
跟「天天紅的燈等於沒有燈」是同一條理由。

所以拆成兩格。判斷「跑過幾班」的真相源是 `_probe/source-health.json`，
而一條從來沒被抓過的來源在那份檔案裡是**缺席**、不是 `runs: 0`——
兩者在 `.get(sid) or 0` 底下自然合流，這一次剛好是對的。

`capability_claims_line` 的 `runs` 參數**沒有預設值**，理由同
`pulse-gate.evaluate`：給了 `None` 預設，忘了傳的呼叫端會靜靜退回舊行為，
而所有測試都會照樣綠——因為它們都有傳。selftest 直接釘簽章。

沒有人在等第一班的時候，第四格**不印**。恆常印一個 0 只會讓這一行變長
...
| 幾條還沒跑過第一班 | **這不是壞消息** | 混進上一格，補來源就會製造假的壞消息 |
