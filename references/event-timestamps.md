...
| （判準上線後實跑，改用完整時間戳而非只比日期）`coverage: backfilled` | **34 / 36** |
...
|---|---|---|
| 證據 1 筆 | 〈證據〉 | 無標籤 |
| `coverage` 不是 `observed` | 〈證據〉＋回填說明 | 無標籤 |
| `coverage: observed` 且證據 ≥2 筆 | 〈發展歷程〉 | 「起點」 |

**只有第三列會出現「起點」。** 一個只有一項的清單不叫歷程，它唯一的那一項也不叫起點。

回填說明是一句固定文案，不是散文：「這些證據是首抓時撈回的存量——事情發生時，
我們還沒有在看。」`unknown` 用另一句（「這則的來源何時開始被抓取，我們沒有紀錄」），
**不共用**：「確定是回填」跟「不知道」是兩件事。

### 為什麼沒有〈首抓快照〉那一列

初稿有第四列「證據 ≥2 筆但全部來自同一次首抓 → 〈首抓快照〉」。實作時拿掉，兩個理由：

1. **`backfilled` 已經涵蓋它。** 2026-07-27 實測 36 則已發布事件，「所有證據來源的
   `first_fetch_at` 相同」的有 35 則、`backfilled` 的有 34 則——兩個集合幾乎重合，
   而重合的部分本來就走〈證據〉那一列，區別只在標題文字。
2. **要分辨它，render 得自己再讀一次 `_probe/state.json`。** 那會讓
   `first_fetch_at` 多一個消費者、多一份「怎麼算才對」的實作——為了一個幾乎不改變
   輸出的區別付這個代價不划算。判準單一實作源優先。

代價寫清楚：**一則 `observed` 但兩筆證據同一班進來的事件，仍然會叫〈發展歷程〉、
仍然會有「起點」。** 那是「兩個來源同時報導」而不是「我們看著它發展」。這一格是
本層已知的鬆處，不是沒想到——真要收緊得先付上面第 2 點的代價。

### 事件時間軸

- 畫一條**觀測起點線**，線以下標明它是回填區。
- `coverage: backfilled` 的卡片帶標記。`unknown` 用另一個字樣，不共用同一個標記——
  「確定是回填」與「不知道」是兩件事。`observed` **不標**：那是預設狀態，
  滿版的標記等於沒有標記。
- 頁首印出回填則數。0 也要印——看不見的時候有兩種意思（沒有回填／這段沒跑），
  而「整條時間軸看起來像持續觀測」正是這一層要修掉的誤會。

### 線畫在哪：只宣稱守得住的那一半

線的位置是**最後一則 `observed` 之後**（事件按日期新到舊排）。這樣它的宣稱才可證：
**線以下沒有任何一則是我們當場看到的。**

反過來畫一條「以上都是觀測到的」是假的——`coverage` 是逐則、逐來源算的，
新加一條來源會讓晚期的事件照樣是 `backfilled`。所以沒有一個全域日期能同時
為兩邊背書，只有下半邊守得住。

一則 `observed` 都沒有時線畫在**最前面**（整條都在線下），不是不畫：不畫等於讓
整條時間軸看起來都是我們追到的。判準只讀 Event 自己的 `coverage`，不另外去讀
`_probe/state.json`——那會讓 `first_fetch_at` 多一個消費者（跟拿掉〈首抓快照〉
那一列同一個理由）。
...
- 第一筆證據**沒有日期**時：不得出現「起點」標籤（連它什麼時候發生都不知道，
  就沒有資格說這件事從那裡開始）。而且缺日期的證據在排序上要排**最後**，不是最前。
  ~~證據全部來自同一 `first_fetch_at`：不得出現「起點」標籤。~~ **2026-07-27 撤回**：
  這條跟下面〈為什麼沒有〈首抓快照〉那一列〉直接衝突，而後者是實作採用的版本。
  一份規格對同一個輸入給兩個答案，比缺一條規則更糟。
- fallback：查不到語料時，標題不得等於 `prettify_source(sid)`、日期不得等於事件日。
- 時間軸：`backfilled` 與 `unknown` 的標記字樣必須不同；`observed` 不掛標籤。
- 時間軸：線畫在最後一則 `observed` **之後**；一則 observed 都沒有時線在最前面。
- 時間軸：標記與線真的印在頁面上（判準對了但沒印出來等於沒判）。
- `coverage` 每班重算：改 `state.json` 的 `first_fetch_at` 後重跑，該格要跟著變
  （證明它不是被寫死的 sticky 欄位）。

## 要新增的 mutation（接在 M84 之後）

**判準層（已隨第一層上線，`lib/coverage.py`）**：

| id | 變異 | 為什麼這格值得釘 |
|---|---|---|
| M85 | `coverage` 的 `unknown` 併進 `observed` | 缺格會靜靜變成「我們當時在看」——往說謊的方向倒 |
| M86 | 某條來源缺 `first_fetch_at` 就跳過它、拿其餘的最小值 | 把「有一條不知道何時開始看」稀釋成「別條看得到就算看得到」 |
| M87 | `observable_from` 改取全體來源最小值 | 幾乎所有事件會變成 `observed`，而「數字變好看」不會讓任何測試變紅 |

**呈現層（已隨第二層上線，`pulse-render.py`）**：

| id | 變異 | 為什麼這格值得釘 |
|---|---|---|
| M88 | `backfilled` 事件照樣發「起點」 | 首抓撈回的舊事件，在畫面上跟真的追到的一模一樣 |
| M89 | 單筆證據照樣叫「發展歷程」並發起點 | 實測 36 則已發布事件有 32 則走這條路 |
| M90 | 證據標題查不到時退回來源名 | 畫面上出現一筆標題是「NVIDIA Blog」的證據 |
| M91 | 證據日期查不到時退回事件日 | 一則事件的所有證據看起來同一天出現 |
| M92 | `unknown` 跟 `backfilled` 共用同一句說明 | 把「我們沒紀錄」講成「我們確定沒在看」 |

**時間軸層（已隨第三層上線，`pulse-render.py`）**：

| id | 變異 | 為什麼這格值得釘 |
|---|---|---|
| M102 | `unknown` 跟 `backfilled` 共用同一個標記 | 把「不知道」印成「確定是回填」 |
| M103 | 一則 `observed` 都沒有時不畫線 | 整條時間軸看起來都是我們追到的——正是這一層要修掉的誤會 |
| M104 | 卡片不掛回填標記 | 判準對了但沒印出來，等於沒判 |
| M105 | 線改畫在最後一則 `observed` **之前** | 線以下混進了我們當場看到的那些，宣稱從可證變成假的 |

**接線層（第二層自我審查後補，`pulse-cluster.py` / `pulse-render.py`）**：判準算對了
但沒有真的接到輸出上——這一族的失效全部是靜默的，畫面看起來完全正常。

| id | 變異 | 為什麼這格值得釘 |
|---|---|---|
| M93 | 已潤稿那條寫檔路徑不寫 `coverage` | 這格只會長在沒潤過稿的 Event 上，實測 52 則只有 1 則拿到 |
| M94 | `load_events()` 把 `coverage` 寫死成 `observed` | 往**編造**的方向倒：每一則都會宣稱我們當時在看 |
| M95 | 事件頁把區塊標題寫死回「發展歷程」 | `journey_html` 回傳值被測了，沒有人測頁面有沒有用它 |
| M96 | 缺日期的證據排回最前面 | 「起點／日期未留存」——貼在一筆我們承認什麼都不知道的證據上 |

全部必須被測試**殺掉**，而不是以 crash 代替失敗——M86 第一版就是 crash（`min([])`
的 `ValueError`），判準因此補了一行 `if stamps else None`，讓那個變異以**答錯**收場。
